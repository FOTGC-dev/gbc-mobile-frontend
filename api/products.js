 ...
 try{
  ...
  if(req.method==='GET'){
   const r = await db.query(`
    SELECT id, name,
    COALESCE(price, gc_price::TEXT || ' GC', '0 GC') as price,
    COALESCE(image, image_url, 'https://via.placeholder.com/300') as image,
    COALESCE(stock_text, 'In Stock') as stock,
    COALESCE(display_order, id) as display_order
    FROM products ORDER BY display_order ASC, id ASC
   `);
  ...
  if(req.method==='PUT'){
   ...
   if(b.stock!==undefined) await db.query(`UPDATE products SET stock_text=$1 WHERE id=$2`,[b.stock,id]);
