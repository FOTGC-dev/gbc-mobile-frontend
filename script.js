...
async function handleRegister(e){
 ...
 try{await api('/api/register',{method:'POST',body:JSON.stringify({username:u,email:em,password:p})});showMsg(msg,'success','Created! Redirecting...');setTimeout(()=>location.href='login.html',1000)}catch(err){showMsg(msg,'error',err.error||'Failed - backend may be waking, retry');}
}
...
function renderProducts(containerId,products){
 ...
 cont.innerHTML=products.map(p=>{
  ...
  const img=p.image_url||coinFallback(price,p.name);
...
async function loadAllProducts(){
 ...
 try{
  ...
  const countEl=document.getElementById('prodCount');if(countEl)countEl.textContent=prods.length+' products from Neon';
 }catch(e){
  if(cont)cont.innerHTML='<div style="color:var(--muted);padding:20px">Backend waking up (Render cold start) - wait 20s and refresh</div>';
 }
}
async function loadPanel(){
 const user=getUser();
 if(!user||(user.role!=='owner'&&user.role!=='staff')){document.getElementById('panel-auth').innerHTML='<div style="padding:40px;text-align:center"><h3>Staff / Owner Only</h3><p style="color:var(--muted)">Login as owner / GBC2026!</p><a href="login.html" class="btn btn-white" style="margin-top:12px;display:inline-block">Login</a></div>';return}
 // Load orders
 try{
 ...
 }catch(e){document.getElementById('orders-tab').innerHTML='<div style="color:var(--muted)">Orders API not ready yet - need /api/orders GET in backend</div>'}
 // Load products for management
 try{
  const prods=await api('/api/products');
  document.getElementById('products-tab').innerHTML=prods.map(p=>`<div class="card" style="padding:10px;display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><span>${p.name} - ${p.gc_price} GC</span><button class="btn" onclick="deleteProd('${p.id}')">Delete</button></div>`).join('');
 }catch{}
}
async function deleteProd(id){if(!confirm('Delete product?'))return;try{await api('/api/products/'+id,{method:'DELETE'});alert('Deleted');loadPanel()}catch(e){alert('Delete failed - add DELETE /api/products/:id to backend server.js')}}
async function addProduct(){
 ...
 try{await api('/api/products',{method:'POST',body:JSON.stringify({name,gc_price:price,stock,image_url:img})});alert('Added to Neon Global!');document.getElementById('pName').value='';document.getElementById('pPrice').value='';document.getElementById('pImage').value='';loadAllProducts();loadPanel()}catch(e){alert('Add failed')}
}
